...
vocabList = set(w.lower() for w in nltk.corpus.stopwords.words('english'))
...
def filterAdj(phrasesDict,filename):
	...
	for line_words, count in phrasesDict.items():
		...
		line_words = [Word(word).lemmatize() for word in line_words.split() if(word not in nltk.corpus.stopwords.words('english') and not word.isdigit()) and len(word) > 2]
